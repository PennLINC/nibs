...
OUTPUT_DIR = (
    Path(ARGS.output_dir) if ARGS.output_dir else ICC_DIR / 'selected_metric_summary_figures'
)
...
QC_FILE = (
    Path(ARGS.qc_file)
    if ARGS.qc_file
    else PROJECT_ROOT / 'code' / 'data' / 'manual_qc_modality.tsv'
)

WM_INPUT_GLOBS = [
    str(
        PROJECT_ROOT / 'derivatives/qsirecon/derivatives/qsirecon-*/sub-*/ses-*/dwi/'
        'sub-*_ses-*_*_scalarstats.tsv'
    ),
    str(
        PROJECT_ROOT / 'derivatives/bundle_myelin_stats/sub-*/ses-*/dwi/'
        'sub-*_ses-*_acq-HBCD75_run-01_space-T1w_model-*_scalarstats.tsv'
    ),
]
GM_INPUT_GLOB = str(
    PROJECT_ROOT / 'derivatives/DKTatlas_myelin_stats/sub-*/'
...
WM_NN_CSV = (
    ICC_DIR / f'discriminability_wm_bundles_masked_preferred_median_correlation_{QC_MODE}.csv'
)
...
GM_FILE_RE = re.compile(r'sub-(?P<subject>[^_]+)_(?P<session>ses-[^_]+)_(?P<run>run-[^_]+)_')
...
def qc_passes(
    qc: pd.DataFrame, subject: object, session: object, modalities: tuple[str, ...]
) -> bool:
...
def apply_profile_qc(
    profiles: pd.DataFrame, qc: pd.DataFrame, qc_mode: str = QC_MODE
) -> pd.DataFrame:
    ...
    if qc_mode == 'completeqc':
        ...
        complete_subjects = {
            ...
            if all(
                qc_passes(qc, subject, f'ses-{session:02d}', tuple(modalities))
                for session in (1, 2)
            )
        }
...
def parse_wm_profile(file_path: str, df: pd.DataFrame) -> tuple[str, str, str]:
    ...
    if len(run_candidates) != 1:
        raise ValueError(f'Expected one run in {file_path}, found {sorted(run_candidates)}')
...
def profile_coverage(df: pd.DataFrame, profile_columns: list[str], tissue: str) -> pd.DataFrame:
    region_counts = df.groupby(['metric_label', 'family'])['region'].nunique().rename('n_regions')
    profile_counts = (
...
def mean_profile_spearman(
    ...
    if metric_order is None:
        selected_order = [spec.label for spec in SELECTED_METRICS if spec.label in wide.columns]
    else:
...
def plot_correlation_matrix(
    ...
    if metric_families is None:
        metric_families = {label: SPEC_BY_LABEL[label].family for label in plot_data.index}
    if family_colors is None:
        family_colors = FAMILY_COLORS
    family_by_metric = pd.Series(
        {label: family_colors[metric_families[label]] for label in plot_data.index}
    )
    ...
    plt.setp(grid.ax_heatmap.get_yticklabels(), rotation=0, fontsize=label_fontsize)
...
def load_full_wm_metric_inference():
    ...
    if configured_analysis_dir:
        candidates.append(Path(configured_analysis_dir) / 'compute_icc_from_bundle_stats.py')
    candidates.extend(
        [
...
all_wm_sources = {label: metric_source_image(label) for label in all_wm_order}
...
all_gm_sources = {label: metric_source_image(label) for label in all_gm_order}
...
missing_wm_icc = sorted({spec.label for spec in SELECTED_METRICS} - set(wm_icc['metric_label']))
missing_gm_icc = sorted({spec.label for spec in SELECTED_METRICS} - set(gm_icc['metric_label']))
if missing_wm_icc:
...
def plot_horizontal_violins(
    ...
    order = (
        data.groupby('metric_label')['ICC2_1'].mean().sort_values(ascending=False).index.tolist()
    )
    positions = np.arange(len(order))
    for position, label in zip(positions, order):
        values = data.loc[data['metric_label'] == label, 'ICC2_1'].dropna().to_numpy(dtype=float)
...
def place_scatter_labels(ax: plt.Axes, comparison: pd.DataFrame) -> None:
    ...
    for item in labels:
        ...
        distance = abs(label_axes[0] - point_axes[0]) + abs(label_axes[1] - point_axes[1])
        arrowprops = None
        if distance > 0.065:
            arrowprops = {
                'arrowstyle': '-',
...
def metric_icc_means(data: pd.DataFrame, value_name: str) -> pd.DataFrame:
    return (
        data.groupby(['metric_key', 'metric_label', 'family'], as_index=False)['ICC2_1']
        .mean()
...
nn_values.loc[gm_has_values, 'gm_correct'] = (
    (nn_values.loc[gm_has_values, 'gm_accuracy'] * nn_values.loc[gm_has_values, 'gm_n_profiles'])
    .round()
    .astype('Int64')
)
...
nn_values.loc[wm_has_values, 'wm_correct'] = (
    (nn_values.loc[wm_has_values, 'wm_accuracy'] * nn_values.loc[wm_has_values, 'wm_n_profiles'])
    .round()
    .astype('Int64')
)
