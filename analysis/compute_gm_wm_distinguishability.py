...
def build_metric_colors() -> dict[str, str]:
    ...
    for source_image, base_color in SOURCE_IMAGE_COLORS.items():
        ...
        shade_amounts = [0.0] if len(source_specs) == 1 else np.linspace(0.42, -0.28, len(source_specs))
...
def selected_wm_profiles(input_globs: list[str], qc: pd.DataFrame, qc_mode: str) -> pd.DataFrame:
    ...
    return (
        wm.groupby(["subject_id", "session_id", "metric_key", "metric_label", "family"], as_index=False)[
            "value"
        ]
        .median()
...
def selected_gm_profiles(input_globs: list[str], qc: pd.DataFrame, qc_mode: str) -> pd.DataFrame:
    ...
    return (
        gm.groupby(["subject", "session", "metric_key", "metric_label", "family"], as_index=False)["value"]
        .median()
...
def compute_summary(paired: pd.DataFrame, n_boot: int, seed: int) -> pd.DataFrame:
    ...
    for metric_label, dfg in paired.groupby("metric_label", sort=False):
        ...
        nonzero = diffs[np.isfinite(diffs) & (diffs != 0)]
        try:
            p_value = float(wilcoxon(nonzero, zero_method="wilcox", alternative="two-sided").pvalue) if nonzero.size else np.nan
        except ValueError:
            p_value = np.nan
...
def plot_effect_sizes(summary: pd.DataFrame, out_prefix: Path) -> None:
    ...
    plot_data["source_order"] = plot_data["source_image"].map(SOURCE_IMAGE_ORDER).fillna(len(SOURCE_IMAGE_ORDER))
    plot_data = plot_data.sort_values(["paired_rank_biserial", "source_order", "metric_label"]).reset_index(drop=True)

    y = np.arange(len(plot_data))
    colors = [METRIC_COLORS.get(label, SOURCE_IMAGE_COLORS["Other"]) for label in plot_data["metric_label"]]
    ...
    for index, row in plot_data.iterrows():
        ...
        ax.scatter(value, index, s=52, color=colors[index], edgecolor="black", linewidth=0.45, zorder=3)
    ...
    ax.legend(handles=handles, title="Source image", loc="center left", bbox_to_anchor=(1.02, 0.5), frameon=False)
