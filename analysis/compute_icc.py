...
ATLAS_ROOT = Path('/cbica/projects/nibs/projects/myelin_reliability/code/myelin_atlas')

WM_LABEL = ATLAS_ROOT / 'JHU-ICBM-labels-1mm-MNI152NLin2009cAsym.nii.gz'
LH_LABEL = ATLAS_ROOT / 'MYATLAS_package_new/maps/Volume/vogt_multilabel_lh.nii'
RH_LABEL = ATLAS_ROOT / 'MYATLAS_package_new/maps/Volume/vogt_multilabel_rh.nii'
    ...
    for meas, globpat in patterns.items():
        for fp in deriv_root.glob(globpat):
            ...
            if sub.upper().startswith('PILOT'):
                continue
            rows.append({'measure': meas, 'sub': sub, 'ses': ses, 'path': fp})
...
def parcel_means(data: np.ndarray, labels: np.ndarray):
    ...
    if data.shape != labels.shape:
        raise ValueError(f'Shape mismatch: metric data {data.shape} vs labels {labels.shape}')
...
def process_file(row, wm_atlas, lh_atlas, rh_atlas):
    ...
    wm_labels, lh_labels, rh_labels = get_resampled_atlases(img, wm_atlas, lh_atlas, rh_atlas)
...
def compute_icc_table(df_means: pd.DataFrame) -> pd.DataFrame:
    ...
    for (meas, parc), dfg in df_means.groupby(['measure', 'parcel']):
        ...
        if _HAVE_PG:
            try:
                tab = pd.DataFrame({'targets': subs, 'raters': sess, 'scores': y}).dropna()

                if tab['targets'].nunique() < 2 or tab['raters'].nunique() < 2:
                    icc = np.nan
                else:
                    icc = (
                        pg.intraclass_corr(
...
def plot_icc_outputs(df_icc, figs: Path, tissue_tag: str, tissue_label: str):
    ...
    # Violin
    mean_icc = df_icc.groupby('measure')['ICC2_1'].mean().sort_values(ascending=False)
    measures = []
    data = []
    ...
    legend = [Patch(facecolor=c, edgecolor='black', label=m) for m, c in MODEL_COLORS.items()]
    ax.legend(
        handles=legend,
        title='Model',
...
def main():
    ...
    if args.n_jobs == 1:
        ...
        with ThreadPoolExecutor(max_workers=max(1, args.n_jobs)) as ex:
            futures = {
                ex.submit(process_file, row, wm_atlas, lh_atlas, rh_atlas): row for row in file_rows
            }
