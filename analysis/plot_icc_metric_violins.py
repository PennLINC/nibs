...
def gm_category(parcel: str) -> str:
    ...
    for cat, toks in rules:
        if any(tok in lowercase_parcel for tok in toks):
            return cat
    return 'other'
...
def _plot_violin(
    ...
    metric_order = df.groupby('metric')['ICC2_1'].mean().sort_values(ascending=False).index.tolist()
...
def _plot_metric_scatter(
    ...
    metric_order = comp.mean(axis=1).sort_values(ascending=False).index.tolist()
...
def _retain_common_metrics(
    ...
    if dropped:
        print(
            '[WARN] Excluding selected metrics absent from either GM or WM: ' + ', '.join(dropped),
            flush=True,
        )
    return (
