...
The study acquires multi-modal MRI data (T1w, T2w, MP2RAGE, ihMTRAGE, MESE, MEGRE, dMRI)
...
Scripts in `curation/` are run in order. Shell scripts (`.sh`) run on CUBIC — `03_run_heudiconv.sh`
and `11_reface_anatomicals.sh` are SLURM array jobs, the rest are plain shell.
...
Upstream pipelines (sMRIPrep, QSIPrep, QSIRecon, NextBrain) and the modality scripts are
submitted on CUBIC via the `submit_*.sbatch` scripts in the same directory.
See `processing/README.md` for which steps run on CUBIC versus the lab PC.
...
The `submit_*.sbatch` / `submit_*.sh` scripts in `analysis/` run the longer jobs
(bundle stats, DKT parcel stats, ICC, QC report) on CUBIC.
...
All paths are configured in per-machine YAML files in `configuration/`:
`paths_cubic.yml` for CUBIC and `paths_pc.yml` for the lab PC.
...
Scripts load these files via `configuration.config.load_config()`, which resolves
relative paths against `project_root` and returns a dictionary of absolute paths.
The config name defaults to `'pc'`; pass `load_config('cubic')` to use the CUBIC paths.
