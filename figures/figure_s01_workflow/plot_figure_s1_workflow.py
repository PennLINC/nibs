...
The source is ``configuration/workflow.yml``, where a scalar may depend on
other scalars: R2' comes from R2* and R2, the g-ratios come from ihMT and NODDI,
and chi-separation with a measured R2' comes from MEGRE and R2'.
...
from utils.workflow import (  # noqa: E402
    EDGE_COLOR,
    INK,
    INK_MUTED,
    MODALITIES,
...
def save_figure(fig):
    """Write the canonical workflow figure in the selected run namespace."""

    output_stem = RUN_FIGURES_ROOT / 'figure_s01_workflow' / 'FigureS1'
...
def load_graph():
    ...
    """
    with open(
        os.path.join(_REPO_ROOT, 'configuration', 'workflow.yml'),
        'r',
    ) as fo:
        raw = json.load(fo)
    ...
    if unknown:
        raise ValueError(
            'configuration/workflow.yml depends on names that are neither scalars in '
            f'the file nor modalities in workflow_graph.MODALITIES: {sorted(unknown)}'
        )
    if set(MODALITY_ORDER) != modalities:
        raise ValueError(
            'MODALITY_ORDER is out of date with configuration/workflow.yml:\n'
